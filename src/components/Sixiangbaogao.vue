<template>
  <div id="box">
    <Header class="onepoint">思想报告</Header>
    <div class="one-box">
      <label class="content">
...
<script>
  ...
  export default {
    ...
    methods:{
      gettext(){
        this.$axios.get('getText',{name:'思想汇报'}).then(res=>{
          console.log(res);
          if (res.data=='审核中'){
      ...
      isLogin() {
        this.$axios.get('isLogin').then(res => {
          // console.log(res);
          if (res.data=='未登录'){
            this.$router.push({path:'/Login',query:{mark:"Sixiangbaogao"}});
          }
          // else{
          //   this.gettext();
          // }
      ...
      takeText(){
        this.$axios.post('addText',{name:'思想汇报',img:this.img}).then(res=>{
          // console.log(res);
          if(res.data.type==0){
            this.$router.push({path:'/Checktext'})
